...
export default function BopPrintPreviewPage() {
  ...
  const [selectedItems, setSelectedItems] = useState<Bop[]>([]);
  ...
  useEffect(() => {
    ...
    if (idsJson) {
      ...
      const filtered = bopData.filter(p => ids.includes(p.id));
  ...
  const handlePrint = useReactToPrint({
    ...
    onAfterPrint: () => {
      const newBills = selectedItems.map(p => ({
        ...
        totalAmountDue: Number(getPropertyValue(p, 'Amount Due')) || Number(getPropertyValue(p, 'Permit Fee')) || 0,
    ...
    <div className="flex flex-col min-h-screen bg-slate-50">
      <header className="no-print bg-white border-b p-4 flex justify-between sticky top-0 z-50 shadow-sm">
        <div className="flex items-center gap-4">
          ...
          <h1 className="font-bold">BOP Print Preview ({selectedItems.length})</h1>
        </div>
        <div className="flex items-center gap-4">
        ...
        <div className="space-y-10">
          {selectedItems.map(item => (
            <div key={item.id} className="shadow-2xl bg-white scale-[0.7] sm:scale-[0.8] lg:scale-[1.0] origin-top">
              <PrintableContent data={item} billType="bop" settings={settings} isDemandNotice={isDemandNotice} />
            </div>
          ))}
        </div>
      </main>
