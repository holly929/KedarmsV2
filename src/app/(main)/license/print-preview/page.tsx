...
const BillSheet = React.forwardRef<HTMLDivElement, { licenses: License[], settings: { general: GeneralSettings, appearance: AppearanceSettings }, billsPerPage: number, isCompact: boolean, isDemandNotice: boolean }>(({ licenses, settings, billsPerPage, isCompact, isDemandNotice }, ref) => {
    ...
    if (billsPerPage === 4) {
        ...
        for (let i = 0; i < licenses.length; i += 4) chunks.push(licenses.slice(i, i + 4));
        return (
            <div ref={ref} style={sheetStyle} className="bg-[#ffffff] text-[#000000]">
                {chunks.map((chunk, index) => (
                    <div key={index} className="print-page-break w-[210mm] h-[297mm] mx-auto bg-white grid grid-cols-2 grid-rows-2 box-border overflow-hidden">
                        {chunk.map(l => (
                            <div key={l.id} className="w-full h-full box-border overflow-hidden border-dashed border-gray-300 [&:nth-child(1)]:border-r [&:nth-child(1)]:border-b [&:nth-child(2)]:border-b [&:nth-child(3)]:border-r p-1 break-inside-avoid">
                                <div className="w-full h-full transform scale-[0.98] origin-center">
                                    <PrintableContent data={l} billType="license" settings={settings} isCompact={true} isDemandNotice={isDemandNotice} />
                                </div>
                            </div>
                        ))}
                    </div>
    ...
    if (billsPerPage === 2) {
        const chunks: License[][] = [];
        for (let i = 0; i < licenses.length; i += 2) chunks.push(licenses.slice(i, i + 2));
        return (
            <div ref={ref} style={sheetStyle} className="bg-[#ffffff] text-[#000000]">
                {chunks.map((chunk, index) => (
                    <div key={index} className="print-page-break w-[210mm] h-[297mm] mx-auto bg-white flex flex-col box-border overflow-hidden">
                        {chunk.map((l, ci) => (
                            <div key={l.id} className="h-[148.5mm] w-full box-border overflow-hidden p-1 relative break-inside-avoid">
                                <div className="w-full h-full transform scale-[0.98] origin-center">
                                    <PrintableContent data={l} billType="license" settings={settings} isCompact={isCompact} isDemandNotice={isDemandNotice} />
                                </div>
    ...
    return (
        <div ref={ref} style={sheetStyle} className="bg-[#ffffff] text-[#000000]">
            {licenses.map(l => (
                <div key={l.id} className="print-page-break w-[210mm] h-[297mm] mx-auto bg-white overflow-hidden p-2 break-inside-avoid">
                    <PrintableContent data={l} billType="license" settings={settings} isCompact={isCompact} isDemandNotice={isDemandNotice} />
                </div>
            ))}
        </div>
...
export default function BulkPrintPage() {
  ...
  useEffect(() => {
    ...
    if (idsJson) {
      ...
      const source = licenseData.filter(l => ids.includes(l.id));
      setAllLicenses(source);
    } else if (billIdsJson) {
      const ids = JSON.parse(billIdsJson);
      const source = bills.filter(b => ids.includes(b.id)).map(b => b.propertySnapshot as License);
      setAllLicenses(source);
    }

    setIsDemandNotice(demand);
    setSettings({ general: store.settings.generalSettings || {}, appearance: store.settings.appearanceSettings || {} });
  }, [isClient, licenseData, bills]);
  ...
  useEffect(() => {
    if (allLicenses.length > 0 && isClient) {
        setIsPreparing(true); setRenderedLicenses([]); setProgress(0);
        let current = 0; const chunk = 50;
        const render = () => {
            if (current >= allLicenses.length) { setIsPreparing(false); return; }
            const next = Math.min(current + chunk, allLicenses.length);
            setRenderedLicenses(l => [...l, ...allLicenses.slice(current, next)]);
      ...
      <header className="no-print bg-white border-b p-4 flex justify-between sticky top-0 z-50 shadow-sm">
        <div className="flex items-center gap-4">
            ...
            <div className="flex flex-col">
                <h1 className="text-lg font-bold leading-none">Establishment Batch Print</h1>
                <span className="text-xs text-muted-foreground">{allLicenses.length} documents loaded</span>
            </div>
        </div>
        <div className="flex items-center gap-4">
         ...
         {isPreparing ? (
            <div className="w-full max-w-md mt-20 text-center space-y-4">
                ...
                <div className="space-y-1">
                    ...
                    <Progress value={(progress / allLicenses.length) * 100} className="h-2" />
                    <p className="text-xs text-muted-foreground">{progress} of {allLicenses.length} rendered</p>
                </div>
            </div>
         ) : (
            <div className="space-y-12 w-full flex flex-col items-center">
                <div className="text-center space-y-2">
                    ...
                    <p className="text-muted-foreground max-w-sm mx-auto">Verify the layout below. Documents are painted in a high-fidelity buffer for perfect rendering.</p>
                </div>
